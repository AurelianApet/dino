...
public class arge_cs :  MonoBehaviour
{
...
//Animation controller
	void Update ()
	{
		...
		{
			//Moves
			if (Input.GetKey(KeyCode.LeftShift) && Input.GetKey(KeyCode.W)) anim.SetInteger("State", 3); //Run
			else if (Input.GetKey(KeyCode.W)) anim.SetInteger("State", 1); //Walk
			else if (Input.GetKey(KeyCode.S)) anim.SetInteger("State", -1); //Walk
...
//Motions code
		...
		{
			if (velZ < 0.225f) velZ = velZ + (Time.deltaTime * 0.25F);
...
//Bone rotations, model modification and stick to the terrain
	...
	void LateUpdate()
	{
		...
		if (anim.GetCurrentAnimatorStateInfo (0).IsName("Arge|Sitting")||
		    ...
		    anim.GetCurrentAnimatorStateInfo (0).IsName("Arge|Sleep")||
		    anim.GetCurrentAnimatorStateInfo (0).IsName("Arge|Die")||
		    anim.GetCurrentAnimatorStateInfo (0).IsName("Arge|Die-")) rg.isKinematic=true; else rg.isKinematic=false;
